...
def condition_candidates_from_ir(func: FunctionIR, ops: IrConditionOps) -> list[BranchCandidate]:
    ...
    for index, (stmt, condition, local_names, continuation_facts) in enumerate(_if_conditions_with_aliases(func.statements, {}, set())):
        for suffix, setup, branch_text, facts, branch_facts in condition_setup_alternatives(condition, ops):
            if not setup:
                continue
            if _setup_references_local_root(setup, local_names):
                continue
            ...
            candidates.append(BranchCandidate(
                ...
                target_branch=f"if {branch_text}",
                origin="ir",
                object_paths=_dedup_object_paths([*facts, *continuation_facts]),
...
def condition_setup_alternatives(expr: Expr, ops: IrConditionOps) -> list[tuple[str, list[str], str, list, list[BranchFact]]]:
    ...
    if isinstance(expr, BinaryOp) and expr.op in {"==", "!=", ">", ">=", "<", "<="}:
        ...
        if is_pointer_expr(target_expr):
            return [*_pointer_setup_alternatives(normalized, target, rhs), *_false_setup_alternatives(expr, ops)]
    ...
    if isinstance(expr, UnaryOp) and expr.op == "!":
        ...
        return [
            ...
            (f"false_not_{target}", _non_null_setup(expr.operand, target), f"!!{target}", object_path_facts_from_expr(expr.operand), [BranchFact(target, "!=", "0")]),
        ]

    target = assignable_expr(expr)
    if target is not None:
        return [
            (f"truthy_{target}", _non_null_setup(expr, target), target, object_path_facts_from_expr(expr), [BranchFact(target, "!=", "0")]),
...
def _true_setup_alternatives(expr: Expr, ops: IrConditionOps) -> list[tuple[str, list[str], str, list, list[BranchFact]]]:
    ...
    if isinstance(expr, BinaryOp) and expr.op in {"==", "!=", ">", ">=", "<", "<="}:
        ...
        normalized = BinaryOp(op, target_expr, IntLiteral(0) if _is_null_value(rhs) else expr.right, expr.c_type)
        if is_pointer_expr(target_expr):
            return _pointer_setup_alternatives(normalized, target, rhs)
        value = _value_for_relation(op, rhs, ops)
        setup = _coordinated_field_setup(target, op, rhs, expr.right, ops)
        if not setup:
            setup = [f"{target} = {value};"]
    ...
    target = assignable_expr(expr)
    if target is not None:
        return [(f"truthy_{target}", _non_null_setup(expr, target), target, object_path_facts_from_expr(expr), [BranchFact(target, "!=", "0")])]
...
def _false_setup_alternatives(expr: Expr, ops: IrConditionOps) -> list[tuple[str, list[str], str, list, list[BranchFact]]]:
    ...
    if isinstance(expr, BinaryOp) and expr.op in {"==", "!=", ">", ">=", "<", "<="}:
        ...
        if is_pointer_expr(target_expr):
            return _pointer_setup_alternatives(BinaryOp(false_op, target_expr, IntLiteral(0) if _is_null_value(rhs) else expr.right, expr.c_type), target, rhs, prefix="false_")
    ...
    if isinstance(expr, UnaryOp) and expr.op == "!":
        ...
        if target is None:
            return []
        return [(f"false_not_{target}", _non_null_setup(expr.operand, target), f"!!{target}", object_path_facts_from_expr(expr.operand), [BranchFact(target, "!=", "0")])]
    target = assignable_expr(expr)
    if target is not None:
...
def _pointer_setup_alternatives(
    ...
    if not _is_null_value(rhs):
        return []
    if expr.op == "==":
        value = _null_assignment_value(expr.left)
    elif expr.op == "!=":
        value = _non_null_assignment_value(expr.left)
    else:
        return []
    suffix = f"{prefix}{target}_{relation_name(expr.op)}_{rhs}"
    return [(suffix, [f"{target} = {value};"], f"{target} {expr.op} {rhs}", object_path_facts_from_expr(expr.left), [BranchFact(target, expr.op, rhs)])]
...
def _null_assignment_value(expr: Expr) -> str:
    if is_pointer_expr(expr):
        return "NULL"
    return "0"


def _non_null_assignment_value(expr: Expr) -> str:
...
def _non_null_setup(expr: Expr, target: str) -> list[str]:
    if isinstance(expr, VarRef) and is_pointer_expr(expr):
        return [f"if (!{target}) return 0;"]
    return [f"{target} = {_non_null_assignment_value(expr)};"]
